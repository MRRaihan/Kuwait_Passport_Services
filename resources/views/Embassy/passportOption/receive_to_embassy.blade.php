            ...
            <div class="col-sm-12">
                <div class="page-header-title">
                    ...
                    <ol class="breadcrumb pull-right">
                        <li><a href="{{ route('admin.dashboard') }}">Embassy Panel</a></li>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row">
                            <div class="col-md-3" >
                                ...
                                <select class="form-control" name="onchange_option_id" id="onchange_option_id" onchange="openLink('{{ url('embassy/passport-options/receive-to-embassy') }}/'+'&'+'&'+'&'+'&'+$('#onchange_option_id').val())">
                                    @if(passportOptions()[0])
                                    @foreach (passportOptions() as $key => $passort)
                                        <option value="{{$key}}"   {{ $option == $key ? 'selected' : '' }}   >&nbsp;&nbsp;&nbsp;&nbsp;{{$passort}}</option>
                                    @endforeach
                        ...
                        <form action="{{ route('embassy.passportOption.receiveToEmbassy.store') }}" method="post">
                            @csrf
                            <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                <center><button type="submit" class="btn btn-md btn-success">Receive To Embassy</button></center>
                                <thead>
                                    <tr>
                                        <th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($passport->embassy_status == 2)
                                              <span class="badge badge-pill badge-success"><i class="ti-check"></i>&nbsp;Recieved</span>
                                            @else
    ...
    function undo(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '{{ route('embassy.passportOption.receiveToEmbassy.undo',$option) }}'+'&'+id,
                    headers: {
