...
@section('report_body')
    ...
    <div class="float-right ml-2 mr-3" style="margin-top: 2px;">
      ...
      @if (isset($all_report) && $all_report == true)
          <a class="btn btn-sm  btn-outline-success print-none"  href="{{ route('dataEnterer.allReport.excelExport',$from_date.'&'.$to_date.'&'.$option.'&'.$all_report) }}"><i class="fas fa-file-excel"></i> Excel</a>
      @endif
                    ...
                    @foreach ($passports as $key => $passport)
                        <tr>
                          ...
                          @if ($passport->model_name == 'Renew Passport')
                            <td><a href="{{ route('dataEnterer.renewPassport.show', $passport->id) }}" target="_blank">view</a></td>
                          @endif

                          @if ($passport->model_name == 'Manual Passport')
                            <td><a href="{{ route('dataEnterer.manualPassport.show', $passport->id) }}" target="_blank">view</a></td>
                          @endif

                          @if ($passport->model_name == 'Lost Passport')
                            <td><a href="{{ route('dataEnterer.lostPassport.show', $passport->id) }}" target="_blank">view</a></td>
                          @endif

                          @if ($passport->model_name == 'New Born Baby Passport')
                            <td><a href="{{ route('dataEnterer.newBornBabyPassport.show', $passport->id) }}" target="_blank">view</a></td>
                          @endif
