...
@section('report_body')
    ...
    <div class="float-right ml-2 mr-3 mt-3" style="margin-top: 2px;">
      ...
      @if (isset($shift_to_embassy) && $shift_to_embassy == true)
          ...
          <a class="btn btn-sm  btn-outline-success print-none"  href="{{ route('accountManager.shiftToEmbassyReport.excelExport',$from_date.'&'.$to_date.'&'.$branch_id.'&'.$option.'&'.$shift_to_embassy) }}"><i class="fas fa-file-excel"></i> Excel</a>

          @elseif (isset($receive_from_embassy) && $receive_from_embassy == true)

            <a class="btn btn-sm  btn-outline-success print-none"  href="{{ route('accountManager.receiveFromEmbassyReport.excelExport',$from_date.'&'.$to_date.'&'.$branch_id.'&'.$option.'&'.$receive_from_embassy) }}"><i class="fas fa-file-excel"></i> Excel</a>

          @elseif (isset($delivery_to_branch) && $delivery_to_branch == true)
           <a class="btn btn-sm  btn-outline-success print-none"  href="{{ route('accountManager.deliveryToBranchReport.excelExport',$from_date.'&'.$to_date.'&'.$branch_id.'&'.$option.'&'.$delivery_to_branch) }}"><i class="fas fa-file-excel"></i> Excel</a>

           @elseif (isset($delivery) && $delivery == true)
             <a class="btn btn-sm  btn-outline-success print-none"  href="{{ route('accountManager.deliveryToBranchReport.excelExport',$from_date.'&'.$to_date.'&'.$branch_id.'&'.$option.'&'.$delivery) }}"><i class="fas fa-file-excel"></i> Excel</a>

          @else
          <a class="btn btn-sm  btn-outline-success print-none"  href="{{ route('accountManager.report.excelExport',$from_date.'&'.$to_date.'&'.$branch_id.'&'.$option) }}"><i class="fas fa-file-excel"></i> Excel</a>
      @endif
    ...
    <!-- Optional JavaScript; choose one of the two! -->
           <div class="col-md-12">
            <table class="table table-bordered">
                ...
                @endif
                <tr>
                  <td colspan="{{ $option == 1 ? '8' : '7' }}" class="text-right">Total = </td>
                  <td class="text-right">
                       {{ $total_versatilo_fee }}
                  </td>
                  <td class="text-right">
