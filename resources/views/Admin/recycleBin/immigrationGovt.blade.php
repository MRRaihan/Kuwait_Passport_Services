...
@push('title')
Immigration Govt. Service Table
@endpush
@push('datatableCSS')
    <!-- DataTables -->
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-header-title">
                        <h4 class="pull-left page-title">Immigration Govt. Service</h4>
                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading">
                            <span class="panel-title">Immigration Govt. Service Table</span>
                        ...
                        <div class="panel-body">
                            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                                ...
                                <tbody>
                                    @foreach ($immigrationGovementServices as $immigrationGovementService)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ $immigrationGovementService->name }} {{ $immigrationGovementService->last_name }}</td>
                                            <td>{{ $immigrationGovementService->kuwait_phone }}</td>
                                            <td>
                                                @foreach (json_decode($immigrationGovementService->service_taken) as $item)
                                                <span class="badge badge-primary">{{ get_other_service_fee_name_by_id($item) }}</span>
                                                @endforeach
                                            </td>
                                            <td>{{ $immigrationGovementService->total_fee }}</td>
                                            <td>{{ \Carbon\Carbon::parse($immigrationGovementService->created_at)->format('Y-m-d') }}</td>
                                            <td style="width: 150px">
                                                <button class="btn btn-info" onclick="restore(this)"
                                                    value="{{ route('admin.recycleBin.other.otherServiceRestore',$immigrationGovementService->id.'&'.'3') }}"><i class="mdi mdi-backup-restore"></i> Restore
                                                </button>
                                                <button class="btn btn-danger" onclick="permanent_delete(this)"
                                                    value="{{ route('admin.recycleBin.other.otherServicePermanentDelete',$immigrationGovementService->id.'&'.'3') }}"><i class="fa fa-trash"></i> Delete Permanently
                                                </button>
                                            </td>
                                        </tr>
                                    @endforeach
