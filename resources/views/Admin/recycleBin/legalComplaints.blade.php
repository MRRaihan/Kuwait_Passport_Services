                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class="panel-body">
                            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                                ...
                                <tbody>
                                    @foreach ($legalComplaintsServices as $legalComplaintsService)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td>{{ $legalComplaintsService->name }} {{ $legalComplaintsService->last_name }}</td>
                                            <td>{{ $legalComplaintsService->kuwait_phone }}</td>
                                            <td>
                                                @foreach (json_decode($legalComplaintsService->service_taken) as $item)
                                                <span class="badge badge-primary">{{ get_other_service_fee_name_by_id($item) }}</span>
                                                @endforeach
                                            </td>
                                            <td>{{ $legalComplaintsService->total_fee }}</td>
                                            <td>{{ \Carbon\Carbon::parse($legalComplaintsService->created_at)->format('Y-m-d') }}</td>
                                            <td style="width: 150px">
                                                <button class="btn btn-info" onclick="restore(this)"
                                                    value="{{ route('admin.recycleBin.other.otherServiceRestore',$legalComplaintsService->id.'&'.'2') }}"><i class="mdi mdi-backup-restore"></i> Restore
                                                </button>
                                                <button class="btn btn-danger" onclick="permanent_delete(this)"
                                                    value="{{ route('admin.recycleBin.other.otherServicePermanentDelete',$legalComplaintsService->id.'&'.'2') }}"><i class="fa fa-trash"></i> Delete Permanently
                                                </button>
                                            </td>
                                        </tr>
                                    @endforeach
