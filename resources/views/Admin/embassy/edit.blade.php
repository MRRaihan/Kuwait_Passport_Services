<form enctype="multipart/form-data" id="edit-form">
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="name" name="name" placeholder="Enter Fist Name" value="{{ $user->name }}">
    </div>
    ...
    <button type="submit" id="btnUpdateAccountManager" class="btn btn-info">Update Embassy</button>


  </form>

  <script>
    $(document).on('click','#btnUpdateAccountManager',function (event) {
     event.preventDefault();
     var form = $('#edit-form')[0];
     var formData = new FormData(form);
    //  console.log(formData);
     formData.append('_method','PUT');
    //  formData.append('old_img','PUT');
     ...
     $.ajax({
         url: "{{ route('admin.embassy.update',$user->id) }}",// your request url
         data: formData,
         processData: false,
         contentType: false,
         type: 'POST',
