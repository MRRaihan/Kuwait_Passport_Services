                    ...
                    <div class="panel-body">
                        ...
                        <div class="row">
                            <div class="col-md-3" >
                                ...
                                <select class="form-control" name="onchange_option_id" id="onchange_option_id" onchange="openLink('{{ url('branch-manager/passport-options/shift-to-admin') }}/'+'&'+'&'+'&'+'&'+$('#onchange_option_id').val())">
                                    @if(passportOptions()[0])
                                    @foreach (passportOptions() as $key => $passort)
                                        <option value="{{$key}}"   {{ $option == $key ? 'selected' : '' }}   >&nbsp;&nbsp;&nbsp;&nbsp;{{$passort}}</option>
                                    @endforeach
                        ...
                        <form action="{{ route('branchManager.passportOption.shiftToAdmin.store') }}" method="post">
                            @csrf
                            <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                <center><button type="submit" class="btn btn-md btn-info">Shift To Admin</button></center>
                                <thead>
                                    <tr>
                                        <th>
                                        ...
                                        <td>
                                         @if ($passport->branch_status != 3)
                                            ...
                                            @if($passport->embassy_status >= 1 )
                                                <span class="badge badge-pill badge-success">Passport Possessing</span>
                                                @else
    ...
    function undo(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    method: 'POST',
                    url: '{{ route('branchManager.passportOption.shiftToAdmin.undo',$option) }}'+'&'+id,
                    headers: {
