                    ...
                    <div class="panel-body">
                        ...
                        <div class="row" style="margin-bottom: 20px;">
                            <div class="col-md-3" >
                                ...
                                <select class="form-control" name="onchange_option_id" id="onchange_option_id" onchange="openLink('{{ url('branch-manager/passport-options/delivery') }}/'+'&'+'&'+'&'+'&'+$('#onchange_option_id').val())">
                                    @if(passportOptions()[0])
                                    @foreach (passportOptions() as $key => $passort)
                                        <option value="{{$key}}"   {{ $option == $key ? 'selected' : '' }}   >&nbsp;&nbsp;&nbsp;&nbsp;{{$passort}}</option>
                                    @endforeach
                        ...
                        <form action="{{ route('branchManager.passportOption.delivery.store') }}" method="post">
                            @csrf
...
<script>
    function searchOptions() {
         window.open("{{ url('branch-manager/passport-options/delivery') }}/"+$('#emirates_id').val()+"&"+$('#mobile').val()+"&"+$('#from_date').val()+"&"+$('#to_date').val()+"&"+$('#option_id').val(),"_parent");
    }
