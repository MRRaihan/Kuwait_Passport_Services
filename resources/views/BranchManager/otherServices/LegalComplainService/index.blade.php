    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-header-title">
                        <h4 class="pull-left page-title">Legal and Complaints Service</h4>
                        <ol class="breadcrumb pull-right">
                            <li><a href="{{ route('branchManager.dashboard') }}">Branch Manager Panel</a></li>
                            <li class="active">Legal and Complaints Service</li>
                        </ol>
            ...
            <div class="row text-right">
                <div class="col-sm-12">
                    @if ($flag != 0)
                    <a class="btn btn-success btn-sm" href="{{ route('branchManager.legalComplaintsService.create') }}"
                ...
                <div class="col-md-12">
                    <div class="panel panel-primary">
                        <div class="panel-heading" style="background-color: #01ba9a !important;">
                            <span class="panel-title">Legal and Complaints Service
                        ...
                        <div class="panel-body">
                            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                                ...
                                <tbody>
                                    @foreach ($takenLegalComplaintsServices as $key => $legalService)
                                <tr>
                                    <td>{{ ++$key }}</td>
                                    <td>{{ $legalService->name }} {{ $legalService->last_name }}</td>
                                    <td>{{ $legalService->kuwait_phone }}</td>
                                    <td>
                                        @foreach (json_decode($legalService->service_taken) as $item)
                                        <span class="badge badge-primary">{{ get_other_service_fee_name_by_id($item) }}</span>
                                    @endforeach
                                    </td>
                                    <td>{{ $legalService->total_fee }}</td>
                                    <td>{{ \Carbon\Carbon::parse($legalService->created_at)->format('Y-m-d') }}</td>
                                    <td style="width: 150px">
                                        <a href="{{ route('branchManager.legalComplaintsService.show',$legalService->id) }}"
                                            target="_blank" class="btn btn-primary"><i
                                                class="fa fa-eye"></i></a>

                                        <a href="{{ route('printReceipt',[$legalService->id,"legel_service"]) }}" target="_blank" class="btn btn-success"><i class="fa fa-print"></i>Receipt</a>

                                        <a href="{{ route('printSticker',[$legalService->id,"legel_service"]) }}" target="_blank" class="btn btn-info"><i class="fa fa-print"></i>Sticker</a>

                                        <button class="btn btn-danger" onclick="delete_function(this)" value="{{ route('branchManager.legalComplaintsService.destroy',$legalService) }}"><i class="fa fa-trash"></i> </button>
                                    </td>
                                </tr>
                                @endforeach
