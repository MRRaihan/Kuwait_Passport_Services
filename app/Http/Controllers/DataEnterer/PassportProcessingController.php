...
class PassportProcessingController extends Controller
{
    ...
    {
        return $this->searchReceiveDelivered('&&&&0');
    }

    public function searchReceiveDelivered($data)
    {
        $civil_id = explode('&', $data)[0] ?? '';
        $mobile = explode('&', $data)[1] ?? '';
        $from_date = explode('&', $data)[2] ?? '';
        ...
        $data = [
            ...
            ->where('new_mrp_passport_no','!=', null)
            ->where('de_id_for_bio', Auth::user()->id)
            ->orderBy('id', 'desc')
            ->get()
        ];
        return view('DataEnterer.passportProcessing.delivered', $data);
    }
